  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="Mehr">
      <Link
        to="/settings/pair"
        onClick={onClose}
        className="mb-6 flex items-center gap-4 rounded-[28px] border border-border/80 bg-[linear-gradient(145deg,var(--color-pastel-1),var(--color-pastel-2))] p-4 shadow-xs transition duration-280 hover:-translate-y-0.5"
      >
              ...
              {group.modules.map((mod) => (
                <li key={mod.key}>
                  <Link
                    to={mod.path}
                    onClick={onClose}
                    className="flex min-h-14 items-center justify-between gap-3 rounded-[22px] border border-border/80 bg-surface px-4 py-3 transition duration-280 active:scale-[0.99] hover:bg-surface-soft"
                  >
                    <span>

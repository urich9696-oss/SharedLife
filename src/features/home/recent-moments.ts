...
export function momentOccurredAt(
  entity: EntityRow,
  details?: EntityDetailRow[],
): string {
  return (
    capturedAtFromDetails(entity.id, details) ??
    entityDateIso(entity) ??
    entity.created_at
  )
}
...
export function selectRecentMoments(input: SelectRecentMomentsInput): RecentMomentCard[] {
  ...
  const moments = input.entities.filter(
    (e) => e.entity_type === 'moment' && !e.deleted_at,
  )
